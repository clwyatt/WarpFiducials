...
template <class T>
T
GetDeterminant(const vnl_matrix<T> & m)
{
              ...
              vnl_matrix<T> & resultM)
{
  ...
  while( (niter <= niterMax) && (energy > precision) )
    ...
    const T gamma = vcl_pow(vcl_abs(GetDeterminant(Mk)), -1.0/(2.0*n));
    const T gamma2 = gamma*gamma;
    invMk = vnl_matrix_inverse<T>(Mk);
                 ...
                 const int numApprox)
{
  ...
  return interm2*vnl_matrix_inverse<T>(interm3);
}
               ...
               const int numApprox)
{
  ...
  case 2:
    {
    vnl_matrix<T> sqr = interm*interm;
  ...
  interm = interm2*vnl_matrix_inverse<T>(interm3);
                   ...
                   const T precision)
{
  ...
  while ((energy>precision) && (niter <= niterMax))
    {
    invBar = vnl_matrix_inverse<T>(bar);
